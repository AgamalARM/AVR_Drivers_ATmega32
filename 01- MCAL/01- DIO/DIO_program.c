...
DIO_ErrorStatus DIO_enumGetPinValue      ( u8 Copy_u8Port , u8 Copy_u8Pin, u8 * Copy_u8ptrData ){
	...
	if ( Copy_u8Pin >= DIO_PIN0 && Copy_u8Pin <= DIO_PIN7 ) {
	switch ( Copy_u8Port ){
			case DIO_PORTA: *Copy_u8ptrData = GET_BIT(PINA_REG, Copy_u8Pin) ;  break; 
			case DIO_PORTB: *Copy_u8ptrData = GET_BIT(PINB_REG, Copy_u8Pin) ;  break; 
			case DIO_PORTC: *Copy_u8ptrData = GET_BIT(PINC_REG, Copy_u8Pin) ;  break; 
			case DIO_PORTD: *Copy_u8ptrData = GET_BIT(PIND_REG, Copy_u8Pin) ;  break; 
			default       : LOC_enumState   = DIO_NOK                       ;  break;
		}
	}else{ LOC_enumState = DIO_NOK ; }
